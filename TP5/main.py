...
Z2 = [i/10 for i in range(-30,101)]

# hyperplan equation (in 2D)
def h(wop,x):
    return -(wop[1]*x + wop[0])/wop[2]
# hyperplan equation (in 2D) for non linear transformer
def htr(wop,data,q):
    data_ = np.asarray([psy(x,q) for x in data])
    return sum([wop[i] * data_[i] for i in range(len(wop))])
